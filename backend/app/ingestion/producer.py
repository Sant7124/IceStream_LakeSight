...
class KafkaTransactionProducer:
    ...
    def __init__(self, topic: str):
        ...
        if security_protocol != "PLAINTEXT":
            ...
            env_ca = os.getenv("KAFKA_CA_LOCATION")
            if env_ca and os.path.exists(env_ca):
                conf['ssl.ca.location'] = env_ca
            elif os.path.exists(os.path.join("secrets", "ca.pem")):
                conf['ssl.ca.location'] = os.path.join("secrets", "ca.pem")
            elif os.path.exists(os.path.join("..", "secrets", "ca.pem")):
                conf['ssl.ca.location'] = os.path.join("..", "secrets", "ca.pem")
            elif certifi:
